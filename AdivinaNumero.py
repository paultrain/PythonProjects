...
def adivinar(x):
    ...
    while adivinar != random_numero:
        ...
        if adivina < random_numero:
            ...
            print(f"Felicidades!!!. Adivinaste el numero {random_numero} correctamente.")
...
def computadora_adivina(x):
    ...
    while devolucion != 'c':
        ...
        devolucion = input(f"Es {adivina} muy alto (A), muy bajo(B), o correcta(C)?").lower()
        if devolucion == 'a':
            alto = adivina - 1
        elif devolucion == 'b':
            bajo = adivina + 1
        else:
            print(f"Bien! La computadora adivinó tu numero: {adivina} correctamente!!!")

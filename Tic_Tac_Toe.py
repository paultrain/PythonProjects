...
def EnterMove(board):
	ok = False  # Lo necesitamos para entrar en bucle
	while not ok:
		move = input("Escoge la casilla : ")
		ok = len(move) == 1 and move >= '1' and move <= '9'  # Es valida?
		if not ok:
			# no lo es - haz la entrada nuevamente
			print("Jajaja mal movimiento - repite tu entrada!")
			continue
...
def VictoryFor(board, sign):
	...
	for rc in range(3):
		...
		if board[0][rc] == sign and board[1][rc] == sign and board[2][rc] == sign:  # check column rc
			return who
		if board[rc][rc] != sign:  # checar 1ra lista diagonal
			cross1 = False
		if board[2 - rc][2 - rc] != sign:  # checar 2da diagonal
			cross2 = False
	if cross1 or cross2:
		return who
	return None
...
board = [[3 * j + i + 1 for i in range(3)]
         for j in range(3)]  # crea un tablero vacío
